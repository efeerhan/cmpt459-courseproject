...
def main():
    location = pd.read_csv('data/location_2021.csv', usecols=['Country_Region','Lat','Long_']) \
        .rename(columns=({'Country_Region': 'country','Lat': 'latitude','Long_': 'longitude'})).dropna()
    train = pd.read_csv('data/cases_2021_train.csv', usecols=['country','latitude','longitude']).dropna()
    test = pd.read_csv('data/cases_2021_test.csv', usecols=['country','latitude','longitude']).dropna()
